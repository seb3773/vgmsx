...
internal_refresh (PSG * psg)
{
  if (psg->quality)
  {
    ...
    psg->realstep = (e_uint32) ((1ULL << 31) / psg->rate);
    psg->psgstep = (e_uint32) ((1ULL << 31) / (psg->clk / 8));
    psg->psgtime = 0;
  }
  else
  {
    psg->base_incr =
      (e_uint32) ((uint64_t) psg->clk * (1 << GETA_BITS) / (8 * psg->rate));
  }
}
...
PSG_calc_stereo (PSG * psg, e_int32 **out, e_int32 samples)
{
  ...
  for (i = 0; i < samples; i ++)
  {
    ...
    else
    {
      ...
      bufMO[i] = (e_int32) (((int64_t) psg->snext[0] * (psg->psgstep - psg->psgtime)
                           + (int64_t) psg->sprev[0] * psg->psgtime) / psg->psgstep);
      bufRO[i] = (e_int32) (((int64_t) psg->snext[1] * (psg->psgstep - psg->psgtime)
                           + (int64_t) psg->sprev[1] * psg->psgtime) / psg->psgstep);
    }
  }
}
