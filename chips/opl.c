// IMPORTANT: This file is not meant to be compiled. It's included in
// adlibemu_opl?.c.
...
// static fltype recipsamp;	// inverse of sampling rate		// moved
// to OPL_DATA
...
INLINE void operator_advance_drums(OPL_DATA *chip, op_type *op_pt1, Bit32s vib1,
                                   op_type *op_pt2, Bit32s vib2,
...
static void change_decayrate(OPL_DATA *chip, Bitu regbase, op_type *op_pt) {
  ...
  if (decayrate) {
    ...
    fltype f =
        (fltype)(-7.4493 * decrelconst[op_pt->toff & 3] * chip->recipsamp);
...
static void change_frequency(OPL_DATA *chip, Bitu chanbase, Bitu regbase,
                             op_type *op_pt) {
  Bit32u frn;
  Bit32u oct;
  Bit32u note_sel;
  ...
  if (!(chip->adlibreg[ARC_TVS_KSR_MUL + regbase] & 0x10))
    op_pt->toff >>= 2;

  // 20+a0+b0:
  op_pt->tinc =
      (Bit32u)((((fltype)(frn << oct)) *
...
static void enable_operator(OPL_DATA *chip, Bitu regbase, op_type *op_pt,
                            Bit32u act_type) {
...
void *ADLIBEMU(init)(UINT32 clock, UINT32 samplerate,
  ...
  for (i = 15; i >= 0; i--) {
    OPL->frqmul[i] = (fltype)(frqmul_tab[i] * INTFREQU / (fltype)WAVEPREC *
                              (fltype)FIXEDPT * OPL->recipsamp);
  }
  ...
  // say 6.4)
  OPL->vibtab_add = (Bit32u)(VIBTAB_SIZE * FIXEDPT_LFO / 8192 * INTFREQU /
                             OPL->int_samplerate);
  OPL->vibtab_pos = 0;
...
static void adlib_write(void *chip, Bitu idx, Bit8u val) {
  ...
  case ARC_FREQ_NUM: {
    ...
    if (base < 9) {
      ...
      Bitu chanbase;
#if defined(OPLTYPE_IS_OPL3)
      if ((OPL->adlibreg[0x105] & 1) && OPL->op[opbase].is_4op_attached)
        break;
#endif
      ...
      if ((OPL->adlibreg[0x105] & 1) &&
          OPL->op[second_set ? (base + 18) : base].is_4op) {
        change_frequency(chip, chanbase, modbase + 8, &OPL->op[opbase + 3]);
        change_frequency(chip, chanbase, modbase + 3 + 8,
                         &OPL->op[opbase + 3 + 9]);
      }
#endif
    }
    ...
    if (base < 9) {
      ...
      if (val & 32) {
        ...
        if ((OPL->adlibreg[0x105] & 1) && OPL->op[opbase].is_4op) {
          ...
          enable_operator(chip, modbase + 8, &OPL->op[opbase + 3],
                          OP_ACT_NORMAL);
  ...
  case ARC_WAVE_SEL + 0x10: {
    ...
    if ((num < 6) && (base < 22)) {
      ...
      // change waveform
      if (OPL->adlibreg[0x105] & 1)
        OPL->wave_sel[wselbase] =
            val & 7; // opl3 mode enabled, all waveforms accessible
      else
        OPL->wave_sel[wselbase] = val & 3;
...
//	- added parameter "chn" to fix panning for 4-op channels and the Rhythm
//Cymbal
#undef CHANVAL_OUT
#if defined(OPLTYPE_IS_OPL3)
...
void ADLIBEMU(getsample)(void *chip, INT32 **sndptr, INT32 numsamples) {
  ...
  for (cursmp = 0; cursmp < samples_to_process; cursmp += endsamples) {
    ...
    vib_tshift = ((OPL->adlibreg[ARC_PERC_MODE] & 0x40) == 0)
                     ? 1
    ...
    if (OPL->adlibreg[ARC_PERC_MODE] & 0x20) {
      ...
      // TomTom (j=8)
      if (!(OPL->MuteChn[NUM_CHANNELS + 2]) &&
          OPL->op[8].op_state != OF_TYPE_OFF) {
        cptr = &OPL->op[8];
        if (cptr[0].vibrato) {
          vibval3 = vibval_var1;
          for (i = 0; i < endsamples; i++)
      ...
      if ((OPL->op[7].op_state != OF_TYPE_OFF) ||
          (OPL->op[16].op_state != OF_TYPE_OFF) ||
        ...
        // calculate channel output
        cptr = &OPL->op[0]; // set cptr to something useful (else it stays at
                            // op[8])
        for (i = 0; i < endsamples; i++) {
          Bit32s chanval;
          ...
          // chanval = (OPL->op[7].cval + OPL->op[7+9].cval +
          // OPL->op[8+9].cval)*2; CHANVAL_OUT(0)
    ...
    for (cur_ch = max_channel - 1; cur_ch >= 0; cur_ch--) {
      ...
      if ((OPL->adlibreg[ARC_PERC_MODE] & 0x20) && (cur_ch >= 6) &&
          (cur_ch < 9))
        continue;
          ...
          if (OPL->adlibreg[ARC_FEEDBACK + k + 3] & 1) {
            ...
            if (cptr[0].op_state != OF_TYPE_OFF) {
              ...
              for (i = 0; i < endsamples; i++) {
                ...
                chanval = cptr[0].cval;
                CHANVAL_OUT(3) // Note: Op 1 of 4, so it needs to use the
                               // panning bits of Op 4 (Ch+3)
              }
            }
